...
class Agent():
    ...
    def generate_random_pose(self, pose, prob_mean, success_rate, bad_pose):
        random_values = [0] * 4
        ...
        while True:
            ...
            for idx in range(1,4):
                if prob_mean[idx] == pose[idx]:
                    ...
                    while True:
                        tmp = stats.norm.rvs(loc=prob_mean[idx], scale=angle_gap * success_rate, size=1)[0]
                        if (pose[idx] - angle_gap < tmp and tmp < pose[idx] + angle_gap):
                            break
                    random_values[idx] = tmp
            # # suppress unwanted camera angle movement
            # random_values.append(np.random.uniform(pose[3] - angle_gap, pose[3] + 0.2 * angle_gap))
            for each in bad_pose:
                # print(np.linalg.norm(np.array(random_values[1:]) - np.array(each[1:])))
                if np.linalg.norm(np.array(random_values) - np.array(each)) < 0.05:
                    prob_mean = pose.copy()
                    success_rate = success_rate_origin
                    not_bad = False
            if not_bad:
                break
        return random_values, success_rate
...
def main():
    ...
    try:
        ...
        while True:
            ...
            i +=1
            if distance < threshold * 1.4:
                angle_gap = joint_angle_delta
            else:
                angle_gap = joint_angle_delta * 0.6
            ...
            for _ in range(random_state_gen_num):
                success_rate *= 0.7
                goal_joint_angle[:4], success_rate = agent.generate_random_pose(pose, prob_mean, success_rate, bad_pose)
                teleop_keyboard.send_goal_joint_space(path_time)
                wait_arrive()
                distance = np.sqrt(np.float64(tvec[0])**2 + np.float64(tvec[1])**2 + np.float64(tvec[2])**2)
