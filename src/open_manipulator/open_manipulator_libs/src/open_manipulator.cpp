...
void OpenManipulator::initOpenManipulator(bool using_actual_robot_state, STRING usb_port, STRING baud_rate, float control_loop_time)
{
  ...
  *****************************************************************************/
  ...
  addJoint("joint4_2",  // my name
            ...
            Y_AXIS,    // axis of rotation
            15,        // actuator id
            1.7,       // max joint limit (2.0 rad)
            -1.7,      // min joint limit (-1.8 rad)
            1.0,       // coefficient
            1.4327573e-01,                                                        // mass
            math::inertiaMatrix(8.0870749e-05, 0.0, -1.0157896e-06,
                                7.5980465e-05, 0.0,
  ...
  addTool("gripper",  // my name
          "joint4_2",   // parent name
          math::vector3(0.126, 0.0, 0.0),                 // relative position
          math::convertRPYToRotationMatrix(0.0, 0.0, 0.0), // relative orientation
          16,         // actuator id
          0.003,      // max gripper limit (0.01 m)
          -0.004,     // min gripper limit (-0.01 m)
...
void OpenManipulator::processOpenManipulator(double present_time)
{
  ...
  // JointWaypoint goal_joint4_2_value = getJoint4_2GoalValue();  // 새로운 관절에 대한 목표값 가져오기
  ...
  // if(goal_joint4_2_value.size() != 0) sendJoint4_2ActuatorValue(goal_joint4_2_value);  // 새로운 관절 제어
