...
class Listen(Node):
    def __init__(self):
        ...
        self.subscriptiont_faceidget_ = self.create_subscription(Int32, 'faceid_to_main_givetopic_facenumber', self.faceid_to_main_givetopic_facenum, 10)
...
# 타임순서로 만들기 사람이한명 왔다고 가정한후에 숫자 1을 줄거임 후에 (0~5까지줘야함)
    def faceid_to_main_givetopic_facenum(self, msg):
        global testfaceid
        testfaceid = msg.data
...
#클릭으로 이루어주면된다. 
    ...
    def main_to_minibot_move_start_anyposition_to_home(self):
        msg = String()
        msg.data = 'main_to_minibot_move_start_anyposition_to_home'
        self.publisherminibotanytohome_.publish(msg)
        self.get_logger().info(f'Publising: {msg.data}"')

    def main_to_minibot_move_start_home_to_shoebox(self):
        msg = String()
        msg.data = 'main_to_minibot_move_start_home_to_shoebox'
...
#         self.timer.start(1000) 
        ...
        # self.miniStoHfinbtn.clicked.connect(self.minibotfin)
        # self.roboPfinbtn.clicked.connect(self.roboarmfin)
    
        # self.minibotbtn.clicked.connect(lambda: self.minibotgogo(node))
        # self.minibotHtoSbtn.clicked.connect(lambda: self.minibotStoH(node))
...
# # each button have trigger publihser
 ...
 #pyqt에서 버튼 클릭하면* ui 를 왜쓰는가? 람다써서 connect(lambda: self.{func}(node))
    ...
    def mainmobile(self, node, testfaceid):
        testfaceid_tmp = int(testfaceid)
        if testfaceid_tmp == 0:
            self.lineEdit_4.setText('홈위치에갈것')
            self.lineEdit_5.setText('')
            self.lineEdit_6.setText('')

            self.minibotHtoSbtn.clicked.connect(lambda: self.minibotHtoS(node))
            ...
            self.miniStoHfinbtn.clicked.connect(self.minibotfinAtoHer)  
            ...
            if roboarmfinpick == 'roboarm_to_main_pickupactive_finish':
                ...
                self.lineEdit_4.setText('main_to_minibot_move_start_home_to_shoebox')
                self.roslog.append('main_to_minibot_move_start_home_to_shoebox')
                ...
                if minibotfinHtoS == 'minibot_to_main_move_finish_home_to_shoebox':
                    self.lineEdit_4.setText('minibot_to_main_move_finish_home_to_shoebox')
                    self.roslog.append('minibot_to_main_move_finish_home_to_shoebox')

                    self.lineEdit_5.setText('main_to_roboarm_unleashactive_start')
                    self.roslog.append('main_to_roboarm_unleashactive_start')

                    self.roboUbtn.clicked.connect(self.robounleachact)       
                    ...
                    if roboarmfinunleash == 'roboarm_to_main_unleashactive_finish':
                        ...
                        self.lineEdit_4.setText('main_to_minibot_move_start_shoebox_to_home')
                        self.roslog.append('main_to_minibot_move_start_shoebox_to_home')
                        self.miniStoHbtn.clicked.connect(self.minibotStoH)
                        self.miniStoHfinbtn.clicked.connect(self.minibotfinStoHer)
                        node.minibotfinStoH_callback
                        if minibotfinStoH == 'minibot_to_main_move_finish_shoebox_to_home':
                            self.lineEdit_4.setText('minibot_to_main_move_finish_shoebox_to_home')
                            self.roslog.append('minibot_to_main_move_finish_shoebox_to_home')

                            self.lineEdit_5.setText('shoe -1box in -completed')
                            self.roslog.append('minibot state 0 location-completed')
                            self.lineEdit_6.setText(f'shoe -{str(testfaceid)}-completed')
                            self.roslog.append(f'shoe -{str(testfaceid)}-completed')
                        else:
                            return self.roslog.append('shoeworks not')
